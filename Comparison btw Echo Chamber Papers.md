...
這兩篇 Paper 都在探討 LLM Agent 在 Simulate 社群網路中的 Echo Chamber 表現，並以傳統數值模型作為對照基準。
...
|------|------|------|
| Network Structure | 預設三種靜態 Network | Twitter 真實資料收集 |
| Agent 設計 | Persona（Big Five）+ 短/長期記憶 | 歷史貼文作為 Context |
| Opinion Updates | 文字（SSF）vs 數值（BCM / FJ） | 文字（ChatGPT、GPT4o Mini、Gemini、Gemma、Meta-Llama、Qwen） vs. 數值 (Equation-Based) |
| Network Dynamics | **固定，不變** | **動態 Follow / Unfollow** |
| Validation 指標 | Polarization / GD / NCI | Modularity / Clustering / Path Length / Density |
| Summary | LLM 比傳統數值方法更能呈現 Echo Chamber  |
